...
public class MedicineRepository {
    ...
    public static MedicineRepository getInstance(){
        if(medicineRepository==null) medicineRepository=new MedicineRepository();
    ...
    public List<Medicine> searchMedicines(String name) {
        String query = "select * from medicines where medicineName like ?";
        List<Medicine> medicineList = new ArrayList<>();
        try (Connection connection = NalamDB.getConnection();
             PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, "%" + name + "%");
            try (ResultSet rs = statement.executeQuery()) {
                while (rs.next()) {
                    ...
                    Medicine medicine = new Medicine(medicineName, expiry, price, desc, quantity);
                    medicineList.add(medicine);
                }
            }
        } catch (Exception e) {
